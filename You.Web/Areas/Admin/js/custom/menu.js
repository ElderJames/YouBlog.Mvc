   ...
   selections = [];
    ...
    var Service = {
        getData: function (params, callback) {
            ...
            $.get('/Admin/Menu/All', params, function (data) {dialog.close(); callback(data); });
        },
        update: function (method, callback) {
            ...
            $.ajax({
                url: method == 'Add' ? '/Admin/Menu/Add' : '/Admin/Tag/Edit/' + $('#TagID').val(),
                type: 'POST',
        ...
        Delete: function (id, callback) {
            ...
            $.post("/Admin/Menu/Delete", { TagID: id }, function (data) { dialog.close(); callback(data); })
        },
        Recovery: function (id, callback) {
            ...
            $.post("/Admin/Menu/Recovery", { TagID: id }, function (data) { dialog.close(); callback(data); })
        },
        ...
        setTimeout(function () {
            ...
            Service.getTranslation($('#title').val(), function (data) {
                ...
                if (data.result && data.data[0].length > 0 && keyword != '')
                    $('#subtitle').val(data.data[0].toLocaleLowerCase().replace('[^%&’,;=?$x22]+', '-').replace(/\ +/g, "-"));
            });
        }, 500);
    });
